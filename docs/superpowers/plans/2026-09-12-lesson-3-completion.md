...
- [x] Add failing tests for 84 screenshot paths/files, screenshot placement, and Pages asset packaging in `tests/content.test.js`, `tests/ui.test.js`, and `tests/deployment.test.js`.
- [x] Render `assets/slides/lesson-3/page-01.jpg` through `page-84.jpg`; inspect all newly taught source pages.
- [x] Add `slideScreenshot(note)` in `js/app.js` before each source summary; constrain image width in `styles.css`; include `assets` in `.github/workflows/pages.yml`.
- [x] Create `data/lesson-3/batch-61-63.js`; activate it in `data/deep-learning-lesson-3.js`; set course and tested progress to 63 and update README.
- [x] Run `npm test`, `git diff --check`, and browser image/reading checks; commit only activated data and necessary UI/assets, then push `main`.
